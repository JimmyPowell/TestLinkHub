...
public interface LessonResourceMapper {
    ...
    @Insert("INSERT INTO lesson_resources(uuid, lesson_version_id, name, resources_url, resources_type, sort_order, status, is_deleted, created_at, updated_at) " +
            "VALUES(#{uuid}, #{lessonVersionId}, #{name}, #{resourcesUrl}, #{resourcesType}, #{sortOrder}, #{status}, #{isDeleted}, #{createdAt}, #{updatedAt})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insert(LessonResources lessonResources);

    @Update("UPDATE lesson_resources SET lesson_version_id=#{lessonVersionId}, name=#{name}, resources_url=#{resourcesUrl}, resources_type=#{resourcesType}, sort_order=#{sortOrder}, status=#{status}, is_deleted=#{isDeleted}, updated_at=#{updatedAt} WHERE id=#{id}")
